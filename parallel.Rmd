...
By default this will have access to (and pay for) all the cores on one node.
...
## 5b) Using  SLURM environment variables to control your job
...
Note that *mclapply* *forks* the master process, so all loaded packages and objects will be automatically present in the worker processes.
